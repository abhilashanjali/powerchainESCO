...
contract Regulator {
   ...
   event AddClient (address msgSender,bytes32 msg,uint timestamp);
   event UpdateClientStatus (address msgSender,bytes32 msg,uint timestamp);
   event RegulatoryContractDeployed (address msgSender,bytes32 msg,uint timestamp);
   event EnergyContractDeployed (address msgSender,bytes32 msg,uint timestamp);
   ...
   function acceptProposal(uint _id) constant returns (address _clinicalTrial) {
      ...
      EnergyContractDeployed(msg.sender,"Mined",block.timestamp);
   }
   ...
   function submitCro(bytes32 _name, bytes32 _url) {
      ...
      AddClient(msg.sender,cro.name,block.timestamp);
   }
   ...
   function changeCroStatus(address _addr, uint8 _status) {
      for(uint32 i=0; i<cros.length; i++) {
         if(cros[i].addr == _addr) {
            ...
            {
               UpdateClientStatus(msg.sender,"Approved",block.timestamp);
            }
            else
            {
               UpdateClientStatus(msg.sender,"Rejected",block.timestamp);
            }
            break;
         }
      }
